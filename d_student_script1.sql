/* creating district correlation table */
create table d_student_scores (
	school_type varchar(100),
	positivity_score float);

insert into d_student_scores (student)
values 
((select * from e_student_ids));

select * from m_student
where answered = 'y';

/*create table d_student_scores*/
select * from e_student_scores
union all 
select * from m_student_scores
union all
select * from h_student_scores;

select 'e1_students' as id, avg(e_student_score) as positivity_score from e_student_scores

select * from m_student
where answered = 'y' and id like 'stu-m1-%';

/* cleaning tables */
alter table h_student_scores rename column h_student_score to positivity_score;

alter table e_student_scores drop column answered;

/* create h_student_scores & m_student_scores */
select id, 
((respectful_teachers+
concerned_teachers+
visit_teachers+
...
often_race+
adults_race+
school_race)/29.0) as h_student_score
from h_student_coded;

/* codes m_student + h_student */
select id, answered,
