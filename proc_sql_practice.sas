...
proc sql;
	...
	when students_help_students = 'Almost all the time' then 5 end) students_help_students,	
	...
	when colleagues_support_initiatives = 'Extremely supportive' then 5 end) colleagues_support_initiatives,
	...
	when positive_environment = 'Extremely positive' then 5 end) sas positive_environment,
	...
	(case when leaders_important_staff_satisfied = 'Not important at all' then 1
	when leaders_important_staff_satisfied = 'Slightly important' then 2
	when leaders_important_staff_satisfied = 'Somewhat important' then 3
	when leaders_important_staff_satisfied = 'Quite important' then 4
	when leaders_important_staff_satisfied = 'Extremely important' then 5 end) as leaders_important_staff_satisfied,
	(case when leaders_positive_influence = 'Not important at all' then 1
	when leaders_positive_influence = 'Slightly important' then 2
	when leaders_positive_influence = 'Somewhat important' then 3
	when leaders_positive_influence = 'Quite important' then 4
	when leaders_positive_influence = 'Extremely important' then 5 end) as leaders_positive_influence,
	...
	(case when leaders_effective_rules = 'Not at all effective' then 1
	...
	(case when challenging_families = 'Not at all challenging' then 1
	when challenging_families = 'Slightly challenging' then 2
	when challenging_families = 'Somewhat challenging' then 3
	when challenging_families = 'Quite challenging' then 4
	when challenging_families = 'Extremely challenging' then 5 end) as challenging_families
	from a1_staff
	where answered = 'y'
	union all
	;